...
def aug_resize(img_):
    scale_factor = 1+ np.random.randint(6) #3
...
beta  = 1-alpha
masks_list=[]
dst_path = '/media/ds/WorkSpace/az_tr_1M_aug/Az_clean_ag'

for root,folders, allMasks in os.walk('/media/ds/WorkSpace/DataSet/mask/'):
    for msk in allMasks:
        masks_list.append(root + "/" + msk)

for root,folders, allfiles in os.walk('/media/ds/WorkSpace/az_tr_1M_aug/Az_clean_ag'):  # augment previous results
    for file in tqdm(allfiles):
        fname = root + "/" + file
        # if os.path.exists(dst_path + "/" + "aug_" + file) == True:
        #     continue
        img_ = cv.imread(fname)
        img = img_
        it = ImageTransformer(fname, (img.shape[1], img.shape[0]))
        # r_center_30 = aug_rotate_center(img, 10)
        r_3d_randomRotate = aug_rotate(img, it, th=15+np.random.randint(10), gam = 6+np.random.randint(10), ph=30+np.random.randint(10))
        # r_3d_img_randomRotate_masked  = aug_mask(r_3d_randomRotate , allMasks)
        randomNumber = np.random.randint(100)
        headOrTail = np.random.randint(100)
        if headOrTail >50:
            if img.shape[0] > 80 and randomNumber<70:   #size greater than 80 probabality = 0.70
                result = cv.GaussianBlur(r_3d_randomRotate, (7, 7) ,cv.BORDER_DEFAULT)
            else :
                result = r_3d_randomRotate
        else:
            if img.shape[0] > 150 and randomNumber<60 :
                result = aug_resize(img_)
            else:
                result = r_3d_randomRotate

        # brightness_img = aug_brightness(img, enhanceVal=1.5)
        # brightness_contrast = aug_contrast(result, enhanceVal=0.5)

        # cv.imshow("r_3d_img_45", r_3d_img_randomRotate_masked)
        # cv.imshow("r_center_30", r_center_30)
        # cv.imshow("aug_brightness", brightness_contrast)
        ...
        if randomNumber <20:
            result_ = aug_contrast(result, enhanceVal=0.5)
        else:
            result_ = aug_contrast(result, enhanceVal=0.5)

        cv.imwrite(dst_path + "/" + file, result_)
